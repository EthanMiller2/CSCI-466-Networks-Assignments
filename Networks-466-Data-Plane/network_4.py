...
class Router:
    ...
    def forward(self):
        self.out_intf_L[0].mtu = 45 # set mtu of out interface to 45 
        for i in range(len(self.in_intf_L)): # for each of the packets in the router queue 
            ...
            try:
                ...
                if pkt_S is not None:
                    ...
                    destination = pkt_S[4:5] # get the destination address of the packet
                    outInterface = self.routingTable.get(int(destination)) # query the routing table to find what interface 
                    # the packet should be sent out on
                    self.out_intf_L[int(outInterface)].put(p.to_byte_S(), True) # send the packet 
                    print('%s: forwarding packet "%s" from interface %d to %d with mtu %d' \
                    % (self, p.to_byte_S(), i, outInterface, self.out_intf_L[0].mtu))
            except queue.Full:
